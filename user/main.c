...
void Light_Sensor_Task (void *p_arg)
{
    ...
    while(DEF_TRUE) {
        ...
        // 실제 구현에서는 DMA Transfer Complete Interrupt를 통해 동기화하는 것이 이상적입니다.
    }
}
...
void Status_Task (void *p_arg)
{
    ...
    while(DEF_TRUE) {
        // [안정성 추가] 워치독 카운터 갱신 (시스템이 멈추지 않았음을 하드웨어에 알림)
        // 이 태스크는 최소 1초에 한 번은 실행되므로 여기서 리로드하는 것이 적절함.
        IWDG_ReloadCounter(); 

        // 1. PIR 또는 인터럽트 이벤트 대기 (가장 높은 우선순위)
        p_msg = (uint32_t *)OSQPend(&PirDataQ, 1000, OS_OPT_PEND_BLOCKING, &msg_size, 0, &err); // 1초(1000ms)마다 확인
        ...
        if (idle_counter >= 600) { // 10분 (600초) 이상 무감지
            if (current_status == STATUS_OCCUPIED) {
                ...
                OSTimeDlyHMSM(0, 0, 0, 100, OS_OPT_TIME_HMSM_STK_CHK, &err); // 조도 센서값 읽기 대기
...
void Bluetooth_Task (void *p_arg)
{
    ...
    while(DEF_TRUE) {
        ...
        if (err == OS_ERR_NONE) {
            // 상태 코드를 블루투스로 전송 (예: 0x01=Occupied, 0x02=Vacant, 0x03=Suspicious)
...
void BT_USART_Init(void)
{
    ...
    USART_ITConfig(USART2, USART_IT_RXNE, ENABLE); // 원격 제어 수신 인터럽트 활성화
